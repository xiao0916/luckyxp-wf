...
- 编码规范：本技能目录下 `references/coding_standards.md`（通用基线）
- 项目级编程规范（可选）：`.dwf/coding-specs/*.md`（如 `前端编程规范.md`、`后端编程规范.md`，与所有 spec/迭代共享）
- React 模板（可选）：若新项目且方案基于 React，可参考本技能目录下 `assets/react-pc|react-mobile|shared/`

## 编程范式约定（编码前门禁）

进入编码前，必须先就编程范式与用户达成一致，结果落到项目级 `.dwf/coding-specs/`（与所有 spec/迭代共享）：

- 已有覆盖当前技术栈的规范 → 用 `question` 确认是否沿用（用户可要求修改）。
- 不存在 → 从 `04-技术方案` 探测涉及的端，用 `question` 确认要生成哪几份；逐份协商（第三方库、组件拆分、是否 TypeScript、状态管理、样式、目录结构、命名、接口封装、测试等），写入 `.dwf/coding-specs/{端}编程规范.md` 并逐份确认。
- `.dwf/coding-specs/*.md` 补充 `references/coding_standards.md`，冲突以项目级为准。
- 规范确认后才能进入执行编码。

## 输出

- 目标代码目录中的代码改动
- `.dwf/coding-specs/{端}编程规范.md`（首次或修订时生成；已存在且沿用则不重复生成）
...
- 不在实现清单未确认、`current_step` 不是 `code`、编程范式约定未完成或需求/方案/清单互相冲突时强行编码。
