...
React SPA 项目模板（MO 端），使用 Vite 构建。针对移动端 H5 场景优化。
...
**移动端特性**：
- viewport 适配安全区（`viewport-fit=cover`）
- 触摸友好间距（最小点击区域 44px）
- 禁止长按选中、拨号检测
- 安全区 CSS 变量（`--tab-bar-height`、`--nav-bar-height`）
...
此模板配合 `dwf-orchestrator` 及其 `dwf-coding` 子技能使用。详见 `.opencode/skills/dwf-coding/SKILL.md` 与 `.opencode/skills/dwf-orchestrator/SKILL.md`。