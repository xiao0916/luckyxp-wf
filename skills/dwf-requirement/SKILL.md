...
本技能用于把零散需求整理成 `dev-workflow` 风格的需求文档。它只负责需求捕获与需求文档生成；独立运行时不启动完整开发工作流，工作流模式下按 `.dwf/state.json` 继续当前阶段。
...
2. **目标 spec 目录由运行模式决定，不由本技能臆造。**
   ...
   - 独立模式：`.dwf/state.json` 不存在或不存在满足条件的 active spec。用 `question` 询问用户目标目录，默认提议 `.dwf/specs/{今日日期}-feat-{从用户输入提取的描述}`，由用户确认或修改。
...
- **独立模式**：`.dwf/state.json` 不存在，或不存在满足条件的 active spec。
  - 用 `question` 询问用户目标 spec 目录，默认提议 `.dwf/specs/{今日日期}-feat-{描述}`（描述从用户输入提取），由用户确认或修改。如用户选择别的目录，以用户输入为准。
...
不要一次抛出大量问题。若可以基于上下文合理假设，先明确写入“假设”，并把不确定项放入“待解决问题”。
...
- `version` 初始为 `1.0.0`。
- `revision` 记录“初始版本”。
- 文档必须包含业务流程图，使用 Mermaid 表达主流程；如果信息不足，生成合理的高层流程，并在“待解决问题”中标注需要确认的分支。
...
- 如果用户确认，说明需求文档已完成。
- 如果用户提出修改意见，直接更新 `01-需求文档/需求文档.md`，保持 `status: draft`，并再次请求确认。
...
- 文档内容为中文。
- 文档包含 frontmatter。
- 文档包含背景、目标、用户角色、范围、用例、功能性需求、非功能性需求、约束、依赖、风险、验收标准和待解决问题。
