...
- 阶段间推进、队列变更、拆分、插队、重新开始都必须使用 `question` 获取用户明确确认。
...
1. 读取刚确认的阶段文档，按待确认事项规则检查“待确认项/待确认事项/待解决问题”。
2. 若存在可解析条目，先进入待确认事项逐项确认循环。
3. 将当前阶段加入该 spec 的 `confirmed_stages`。
4. 计算下一阶段：在 `affected_stages` 中找到当前阶段后的下一个值；若无下一个且当前为 `code`，进入迭代完成。
5. 更新该 spec 的 `current_step`、`confirmed_stages`、相关阶段字段、`_meta.json` 与顶层 `updated_at`。
...
在阶段推进前，若刚确认的阶段文档含非空“待确认项/待确认事项/待解决问题”段，按本循环处理。
