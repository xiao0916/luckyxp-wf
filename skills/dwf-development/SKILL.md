...
description: "当用户需要单独执行 DWF 工作流的需求分析、技术方案或实现清单阶段时必须使用本技能。它从已有需求文档和可选设计稿中生成目标 spec 目录下三个文档：`03-需求分析/需求分析文档.md`、`04-技术方案/技术方案.md`、`05-实现清单/实现清单.md`，并请求用户确认后停止。适用于“需求拆解”“技术方案”“实现清单”“生成技术方案”“创建实现清单”“抽离这三个阶段”等场景。可独立运行，也可作为 dwf-orchestrator 编排的工作流的一部分（state.json 中存在 `status: active` 且 `current_step` 为 `breakdown`/`plans`/`todos` 的 spec 时进入工作流模式）。"
---
...
1. **只处理这三个阶段。**
   本技能默认只创建或更新目标 spec 目录下 `03-需求分析/需求分析文档.md`、`04-技术方案/技术方案.md` 和 `05-实现清单/实现清单.md`。不创建其它阶段文档或代码。
...
3. **不要创建完整工作流目录。**
   除目标 spec 目录下 `03-需求分析/`、`04-技术方案/`、`05-实现清单/` 外，不要主动创建 `01-需求/`、`02-设计稿/`。不要修改已有需求文档或设计稿。项目代码目录由 dwf-coding/用户决定，本技能不创建。
...
6. **独立模式下不要创建或推进 `.dwf/state.json`。**
   独立模式只写目标 spec 目录下的三个阶段文档与该 spec 的 `_meta.json`（如本技能创建该 spec）。不要推进完整工作流。只有工作流模式下才在用户确认后更新 state.json 中该 spec 的 `current_step`。
...
- **独立模式**：`.dwf/state.json` 不存在或不存在 `current_step` 为 `breakdown`/`plans`/`todos` 的 active spec。
  ...
  - 如目标 spec 目录不存在，创建它及其下 `03-需求分析/`、`04-技术方案/`、`05-实现清单/`。
  - 在该 spec 目录下写一份初始 `_meta.json`（`name` 为目录名、`status: "active"`、`current_step: "breakdown"`、`is_shared_context: false`、`shared_ref: null` 等）。
...
- **工作流模式**：在 `specs` 数组中找到 `status: "active"` 且 `current_step` 为 `breakdown`/`plans`/`todos` 之一的 spec。目标 spec 目录为 `.dwf/specs/{spec.name}/`。
  - 每个阶段完成后更新该 spec 的 `current_step`、`confirmed_stages`、`selected_plan`（plans 阶段），同步 `_meta.json` 与顶层 `updated_at`。
  - 遵循 dwf-orchestrator 的确认机制。

### 3. 生成需求分析文档

**进入条件**：独立模式，或工作流模式且 spec 的 `current_step` 为 `breakdown`。
...
5. **等待用户确认。**
   ...
   - 只有在用户选择"确认"后：
     - 独立模式：停止。
     - 工作流模式：把该 spec 的 `current_step` 更新为 `"plans"`、把 `breakdown` 追加到 `confirmed_stages`，同步 `_meta.json` 与 `updated_at`。**完成 breakdown 推进后，若该 spec 为初始化 spec（`is_shared_context: true`）且其需求覆盖多个页面/复杂模块，提醒 dwf-orchestrator 主动询问用户是否拆分迭代——本技能不直接拆分，由 orchestrator 拆分。**

### 4. 生成技术方案

**进入条件**：独立模式，或工作流模式且 spec 的 `current_step` 为 `plans`。
...
6. **等待用户选择。**
   ...
   - 只有在用户选择某个方案后：
     - 独立模式：停止。
     - 工作流模式：把该 spec 的 `selected_plan` 记录所选方案、`current_step` 更新为 `"todos"`、把 `plans` 追加到 `confirmed_stages`，同步 `_meta.json` 与 `updated_at`。

### 5. 生成实现清单

**进入条件**：独立模式，或工作流模式且 spec 的 `current_step` 为 `todos`。
...
5. **等待用户确认。**
   ...
   - 只有在用户选择"确认"后：
     - 独立模式：停止。
     - 工作流模式：把该 spec 的 `current_step` 更新为 `"code"`、把 `todos` 追加到 `confirmed_stages`，同步 `_meta.json` 与 `updated_at`。
...
- 只创建或更新了目标 spec 目录下 `03-需求分析/需求分析文档.md`、`04-技术方案/技术方案.md` 和 `05-实现清单/实现清单.md`。
